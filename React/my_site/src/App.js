...
class Header extends React.Component
{
  render ()
  {
    ...
    return <header>Простая шапка</header>
    ...
    <>
      ...
      <div style={{margin: '0 auto', display: 'grid', gap: 10, width: 850 }}>
        {users.length > 0 && users.map(user => {
            return (
            <Card class='AuthorLib' title={`Автор: ${user.name}`} key={Math.random()} style = {{borderColor: '#aF7654', backgroundColor: '#aaaaaa'}}>      
              <p>Email adress: <strong style = {{color: 'red'}}>{user.email}</strong></p>
              <Row gutter={10}>  
               {
                userPosts.filter(post => post.userId === user.id).map(post =>{
                  return(
                      <Card title={post.title} style={{margin:10, borderColor: '#af54384'}} headStyle={{backgroundColor: '#282c34', color: '#fff'}}>
